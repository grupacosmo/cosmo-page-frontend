import { Component, DestroyRef, ElementRef, HostListener, ChangeDetectionStrategy, input, signal, viewChild, computed, effect, inject } from '@angular/core';
...
export class SliderComponent {
    ...
    protected readonly sliderContainer = viewChild.required<ElementRef>('sliderContainer');
    ...
    protected onSlideClick(e: Event): void {
        ...
        if (this.enableFullscreen() && !this.isDragging() && !this.wasRecentTouchSwipe()) {
    ...
    private updateFullscreenSliderPosition(): void {
        ...
        if (modal && modal.nativeElement) {
            const sliderTrack = modal.nativeElement.querySelector('.fullscreen-slider-track');
            if (sliderTrack) {
                const translateX = -this.currentIndex() * 100;
